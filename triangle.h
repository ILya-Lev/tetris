...
public:
  	Triangle(const int row_init, const int column_init, const Field &);
...
Triangle::Triangle(const int row_init, const int column_init, const Field &F)
{
	orient = 0;
	x_init = row_init;
	y_init = column_init;
...
bool Triangle::clear(const Field &F)
{
...
void Triangle::rotLFRT(int *x, int *y)	//makes left rotation 0->1 and right rot 2->3
{
	x[0] = x[2];
	x[3] = x[2]-1;
	x[1] = x[2]+1;
...
void Triangle::rotLSRS(int *x, int *y)	//makes left rotation 1->2 and right rot 1->2
{
	x[0] = x[2]+1;
	x[1] = x[3] = x[2];
...
void Triangle::rotLTRF(int *x, int *y)	//makes left rotation 2->3 and right rot 0->1
{
	x[0] = x[2];
	x[1] = x[2]-1;
	x[3] = x[2]+1;
...
void Triangle::rotLZRZ(int *x, int *y)	//makes left rotation 3->0 and right rot 3->0
{
	x[0] = x[2]-1;
	x[1] = x[3] = x[2];
